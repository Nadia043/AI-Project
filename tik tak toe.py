...
def start_ai_vs_ai():
    ...
    if result == 1:
        result_label.config(text="AI O wins!", fg=TEXT_COLOR)
    elif result == -1:
        result_label.config(text="AI X wins!", fg=TEXT_COLOR)
    else:
        result_label.config(text="It's a draw!", fg=TEXT_COLOR)
    root.update()
...
label = tk.Label(home_frame, text="Select an Option", font=("Helvetica", 24), fg=TEXT_COLOR, bg=BG_COLOR)
label.pack()
...
human_vs_ai_button = tk.Button(
    ...
    borderwidth=5, relief="ridge", width=15, bg=BUTTON_COLOR, fg=TEXT_COLOR
)
...
ai_vs_ai_button = tk.Button(
    ...
    borderwidth=5, relief="ridge", width=15, bg=AI_VS_AI_BUTTON_COLOR, fg=TEXT_COLOR
)
...
def button_click_effect(button):
    ...
    button.config(bg=X_COLOR)  # Change the background color when clicked
    root.update()
    root.after(100, lambda: button.config(bg=BUTTON_COLOR))  # Reset the color after 100ms
...
def show_game_over(result):
    """Displays the game result on the GUI with different colors."""
    if ai_vs_ai_mode:
        if result == 1:
        ...
        if result == 1:
            result_label.config(text="Computer wins!", fg=TEXT_COLOR)
        elif result == -1:
            result_label.config(text="You win!", fg=TEXT_COLOR)
        else:
            result_label.config(text="It's a draw!", fg=TEXT_COLOR)
...
result_label = tk.Label(root, text="", font=("Helvetica", 24), fg=TEXT_COLOR, bg=BG_COLOR)
result_label.pack()
